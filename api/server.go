...
func NewServer(traderManager *manager.TraderManager, database *config.Database, cryptoService *crypto.CryptoService, port int) *Server {
	...
	versionManager := NewVersionManager(database)
	versionChecker := NewVersionChecker(nil) // 稍后设置server引用
	updateInstaller := NewUpdateInstaller(nil) // 稍后设置server引用
	restartManager := NewRestartManager(nil)   // 稍后设置server引用
	...
	// 设置循环引用
	versionChecker.server = s
	updateInstaller.server = s
	restartManager.server = s
...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		// 版本管理路由（公开接口 - 本地部署无需认证）
		api.GET("/version/current", s.handleGetCurrentVersion)
		api.GET("/version/check", s.handleCheckUpdate)
		api.GET("/version/status", s.handleGetUpdateStatus)
		api.GET("/version/progress", s.handleGetUpdateProgress)
		api.POST("/version/download", s.handleDownloadUpdate)
		api.POST("/version/install", s.handleInstallUpdate)
		api.POST("/version/restart", s.handleRestartUpdate)
		api.POST("/version/auto-update", s.handleToggleAutoUpdate)
...
func (s *Server) handleAdminLogin(c *gin.Context) {
	...
	if !adminMode {
		c.JSON(http.StatusForbidden, gin.H{"error": "管理员模式未启用"})
		return
	}
	...
	// 验证管理员密码（这里使用一个简单的硬编码密码，生产环境应该使用更安全的方式）
	// 可以从环境变量或配置文件中读取
	...
	// 获取admin用户
	adminUser, err := s.database.GetUserByID("admin")
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "获取管理员用户失败"})
		return
	}

	// 生成JWT token
	token, err := auth.GenerateJWT(adminUser.ID, adminUser.Email)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "生成token失败"})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"user": gin.H{
			...
			"otp_verified":  adminUser.OTPVerified,
			"is_admin":      true,
		},
		"message": "管理员登录成功",
	})
...
func (s *Server) Start() error {
	...
	log.Printf("  • GET  /api/version/current  - 获取当前版本")
	log.Printf("  • GET  /api/version/check    - 检查更新")
	log.Printf("  • POST /api/version/download - 下载更新")
	log.Printf("  • POST /api/version/install  - 安装更新")
	log.Printf("  • POST /api/version/restart  - 重启应用")
	log.Println()
...
func (s *Server) Shutdown() error {
	log.Printf("🛑 开始优雅关闭服务器...")

	// 停止版本检查器
	if s.versionChecker != nil {
		s.versionChecker.Stop()
		log.Printf("⏹️  版本检查器已停止")
	}
